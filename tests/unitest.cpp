...
TEST_CASE("RoutingTest")
{
    ...
    KINGKONG_ROUTE(app, "/0/<uint>")
    ([&](uint32_t b) {
        B = b;
        return "OK";
    });

    KINGKONG_ROUTE(app, "/1/<int>/<uint>")
    ([&](int a, uint32_t b) {
        A = a;
        B = b;
        return "OK";
    ...
    KINGKONG_ROUTE(app, "/4/<int>/<uint>/<double>/<string>")
    ...
    KINGKONG_ROUTE(app, "/5/<int>/<uint>/<double>/<string>/<path>")
...
TEST_CASE("handler_with_response")
{
    SimpleApp app;
    KINGKONG_ROUTE(app, "/")
    ([](const kingkong::request&, kingkong::response&) {});
} 
...
TEST_CASE("http_method")
{
    ...
    KINGKONG_ROUTE(app, "/").methods("POST"_method,
...
TEST_CASE("multi_server")
{
    static char buf[2048];
    SimpleApp app1, app2;
    KINGKONG_ROUTE(app1, "/").methods("GET"_method,
                                  "POST"_method)([] {
        return "A";
    });
    KINGKONG_ROUTE(app2, "/").methods("GET"_method,
                                  "POST"_method)([] {
        return "B";
    });
...
TEST_CASE("json::wvalue::wvalue(std::[unordered_]map<std::string, json::wvalue>&&)")
{
    ...
    json::wvalue value(std::move(map)); 
...
{
    auto t = kingkong::mustache::compile(R"---(attack of {{name}})---");
...
TEST_CASE("template_load")
{
    ...
    auto t = kingkong::mustache::load("test.mustache");
    kingkong::mustache::context ctx;
    ctx["name"] = "killer tomatoes";
    auto result = t.render(ctx);
    CHECK("attack of killer tomatoes" == result);
...
TEST_CASE("simple_url_params")
{
    ...
    ([&last_url_params](const kingkong::request& req) {
...
TEST_CASE("multipart")
{
    ...
    ([](const kingkong::request& req, kingkong::response& res) {
        ...
        res.add_header("Content-Type", "multipart/form-data; boundary=KINGKONG-BOUNDARY");
        res.body = msg.dump();
        res.end();
    });
...

    {
        ...
        req.url = "/multipart";
        req.add_header("Content-Type", "multipart/form-data; boundary=KINGKONG-BOUNDARY");
...
TEST_CASE("stream_response")
{
    ...
    KINGKONG_ROUTE(app, "/test")
    ([&key_response](const kingkong::request&, kingkong::response& res) {
...
TEST_CASE("websocket")
{
    ...
    KINGKONG_ROUTE(app, "/ws").websocket().onopen([&](websocket::connection&) {
                                          connected = true;
                                          KINGKONG_LOG_INFO << "Connected websocket and value is " << connected;
                                      })
      .onmessage([&](websocket::connection& conn, const std::string& message, bool isbin) {
          KINGKONG_LOG_INFO << "Message is \"" << message << '\"';
      ...
      .onclose([&](websocket::connection&, const std::string&) {
          KINGKONG_LOG_INFO << "Closing websocket";
      });
...
TEST_CASE("zlib_compression")
{
    ...
    // test deflate
    KINGKONG_ROUTE(app_deflate, "/test_compress")
    ([&]() {
        return expected_string;
    });

    KINGKONG_ROUTE(app_deflate, "/test")
    ...
    // test gzip
    KINGKONG_ROUTE(app_gzip, "/test_compress")
    ([&]() {
        return expected_string;
    });

    KINGKONG_ROUTE(app_gzip, "/test")
    ([&](const request&, response& res) {
        res.compressed = false;
...
TEST_CASE("blueprint")
{
    ...
    kingkong::Blueprint bp_not_sub("bp_prefix_second");
    kingkong::Blueprint sub_bp("bp2", "csstat", "cstemplate");
    kingkong::Blueprint sub_sub_bp("bp3");

    KINGKONG_BP_ROUTE(sub_bp, "/hello")
    ([]() {
        return "Hello world!";
    });

    KINGKONG_BP_ROUTE(bp_not_sub, "/hello")
    ([]() {
        return "Hello world!";
    });

    KINGKONG_BP_ROUTE(sub_sub_bp, "/hi")
    ([]() {
        return "Hi world!";
    });
...
TEST_CASE("base64")
{
    ...
    std::string sample_text = "kingkong Allows users to handle requests that may not come from the network. This is done by calling the handle(req, res) method and providing a request and response objects. Which causes kingkong to identify and run the appropriate handler, returning the resulting response.";
    ...
    CHECK(kingkong::utility::base64encode(sample_text, sample_text.length()) == sample_base64);
    CHECK(kingkong::utility::base64encode((unsigned char*)sample_bin, 6) == sample_bin_enc);
    CHECK(kingkong::utility::base64encode_urlsafe((unsigned char*)sample_bin, 6) == sample_bin_enc_url);
    CHECK(kingkong::utility::base64encode((unsigned char*)sample_bin1, 5) == sample_bin1_enc);
    CHECK(kingkong::utility::base64encode((unsigned char*)sample_bin2, 4) == sample_bin2_enc);


    CHECK(kingkong::utility::base64decode(sample_base64, sample_base64.length()) == sample_text);
    CHECK(kingkong::utility::base64decode(sample_bin_enc, 8) == std::string(reinterpret_cast<char const*>(sample_bin)));
    CHECK(kingkong::utility::base64decode(sample_bin_enc_url, 8, true) == std::string(reinterpret_cast<char const*>(sample_bin)));
    CHECK(kingkong::utility::base64decode(sample_bin1_enc, 8) == std::string(reinterpret_cast<char const*>(sample_bin1)).substr(0, 5));
    CHECK(kingkong::utility::base64decode(sample_bin1_enc_np, 7) == std::string(reinterpret_cast<char const*>(sample_bin1)).substr(0, 5));
    CHECK(kingkong::utility::base64decode(sample_bin2_enc, 8) == std::string(reinterpret_cast<char const*>(sample_bin2)).substr(0, 4));
    CHECK(kingkong::utility::base64decode(sample_bin2_enc_np, 6) == std::string(reinterpret_cast<char const*>(sample_bin2)).substr(0, 4));
} // base64
...
TEST_CASE("task_timer")
{
    ...
    kingkong::detail::task_timer timer(io_service);
