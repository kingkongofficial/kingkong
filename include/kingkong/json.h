    ...
    namespace json
    {
        ...
        namespace detail
        {
            ...
            private:
                ...
                friend rvalue kingkong::json::load(const char* data, size_t size);
            };
        ...
        public:
            ...
            {
#ifndef KINGKONG_JSON_NO_ERROR_CHECK
            ...
            bool b() const
            {
#ifndef KINGKONGJSON_NO_ERROR_CHECK
        ...
        inline rvalue load_nocopy_internal(char* data, size_t size)
        {
            struct Parser
            {
                ...
                bool consume(char c)
                {
                    if (kingkong_json_unlikely(*data != c))
                        return false;
                    data++;
                    return true;
                }
                ...
                {
                    if (kingkong_json_unlikely(!consume('"')))
                        return {};
                    char* start = data;
                    uint8_t has_escaping = 0;
                    while (1)
                    {
                        if (kingkong_json_likely(*data != '"' && *data != '\\' && *data != '\0'))
                        {
                            data++;
                        }
                        else if (*data == '"')
                ...
                rvalue decode_list()
                {
                    ...
                    ws_skip();
                    if (kingkong_json_unlikely(*data == ']'))
                    {
                        data++;
                        return ret;
                    }
                ...
                rvalue decode_number()
                {
                    ...
                    while (kingkong_json_likely(state != Invalid))
                    {
                        switch (*data)
                        {
                            case '0':
                            ...
                            case 'E':
                                ...
                                break;
                            default:
                                if (kingkong_json_likely(state == NumberParsingState::ZeroFirst ||
        ...
        {
            friend class kingkong::mustache::template_t;

        public:
            using object =
#ifdef KINGKONGJSON_USE_MAP
...
#undef kingkong_json_likely
#undef kingkong_json_unlikely