...
namespace kingkong {
    namespace websocket
    {
        ...
        {
        public:
            Connection(const kingkong::request& req, Adaptor&& adaptor,
                       std::function<void(kingkong::websocket::connection&)> open_handler,
                       std::function<void(kingkong::websocket::connection&, const std::string&, bool)> message_handler,
                       std::function<void(kingkong::websocket::connection&, const std::string&)> close_handler,
                       std::function<void(kingkong::websocket::connection&)> error_handler,
                       std::function<bool(const kingkong::request&)> accept_handler):
              ...
              open_handler_(std::move(open_handler)), message_handler_(std::move(message_handler)), close_handler_(std::move(close_handler)), error_handler_(std::move(error_handler)), accept_handler_(std::move(accept_handler))
            {
                ...
                start(kingkong::utility::base64encode((unsigned char*)digest, 20));
            }
                        ...
                        adaptor_.socket().async_read_some(
...

                          {
                              ...
                              if (!ec)
                              {
                                  ...
                                  {
#ifndef KINGKONG_ENFORCE_WS_SPEC
                                      has_mask_ = false;
#else
